// chart-loaders-individual.js - Individual chart loading functions for all comparison types
console.log("✅ chart-loaders-individual.js LOADED");

// ========== COUNTRY COMPARISON CHART LOADERS ==========

async function loadDateChartForCountry(canvasId, country) {
    const filters = { ...currentFilters, countries: [country] };
...
async function loadCountryChartForCountry(canvasId, country) {
    const filters = { ...currentFilters, countries: [country] };
...
async function loadUnusualChartForCountry(canvasId, country) {
    ...
    const countryTotals = {};
    countries.forEach(c => {
        countryTotals[c] = dates.reduce((sum, date) => sum + (dateMap[date][c] || 0), 0);
    });
    
    const sortedCountries = Array.from(countries).sort((a, b) => countryTotals[b] - countryTotals[a]);
    
    const datasets = sortedCountries.map(c => {
        const color = getNextColor();
        return {
            label: c,
            data: dates.map(date => dateMap[date][c] || 0),
            borderColor: color,
...
async function loadIpChartForCountry(canvasId, country) {
    const filters = { ...currentFilters, countries: [country] };
...
async function loadAsnChartForCountry(canvasId, country) {
    const filters = { ...currentFilters, countries: [country] };
...
async function loadUsernameChartForCountry(canvasId, country) {
    const filters = { ...currentFilters, countries: [country] };
...
// Load country chart without filtering (used in unusual-source comparison)
async function loadCountryChartUnfiltered(canvasId) {
    const filters = { ...currentFilters };
    const response = await fetch(`${API_URL}/charts/country`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
// Load unusual chart without filtering (used in unusual-source comparison)
async function loadUnusualChartUnfiltered(canvasId) {
    const filters = { ...currentFilters };
    const response = await fetch(`${API_URL}/charts/unusual`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
// Chart loading functions for IP comparison
async function loadIpChartForIp(canvasId, ip) {
    const filters = { ...currentFilters, ips: [ip] };
    const response = await fetch(`${API_URL}/charts/ip`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(filters)
...
async function loadCountryChartForIp(canvasId, ip) {
    console.log(`[loadCountryChartForIp] Called with canvasId=${canvasId}, ip=${ip}`);
    const filters = { ...currentFilters, ips: [ip] };
    console.log(`[loadCountryChartForIp] Sending filters:`, filters);
    ...
    const result = await response.json();
    console.log(`[loadCountryChartForIp] API returned ${result.data?.length || 0} data points`);
    ...
    console.log(`[loadCountryChartForIp] Creating chart with countries:`, sortedCountries, 'totals:', countryTotals);
    ...
    console.log(`[loadCountryChartForIp] Chart created successfully`);
}

async function loadAsnChartForIp(canvasId, ip) {
    console.log(`[loadAsnChartForIp] Called with canvasId=${canvasId}, ip=${ip}`);
    const filters = { ...currentFilters, ips: [ip] };
    console.log(`[loadAsnChartForIp] Sending filters:`, filters);
    ...
    const result = await response.json();
    console.log(`[loadAsnChartForIp] API returned ${result.data?.length || 0} data points`);
    ...
    console.log(`[loadAsnChartForIp] Creating chart with ASNs:`, sortedAsns, 'totals:', asnTotals);
    ...
    console.log(`[loadAsnChartForIp] Chart created successfully`);
}

async function loadUsernameChartForIp(canvasId, ip) {
    console.log(`[loadUsernameChartForIp] Called with canvasId=${canvasId}, ip=${ip}`);
    const filters = { ...currentFilters, ips: [ip] };
    console.log(`[loadUsernameChartForIp] Sending filters:`, filters);
    ...
    const result = await response.json();
    console.log(`[loadUsernameChartForIp] API returned ${result.data?.length || 0} data points`);
    ...
    console.log(`[loadUsernameChartForIp] Creating chart with usernames:`, sortedUsernames, 'totals:', usernameTotals);
    ...
    console.log(`[loadUsernameChartForIp] Chart created successfully`);
}



// Chart loading functions for ASN comparison
async function loadIpChartForAsn(canvasId, asn) {
    const filters = { ...currentFilters, asns: [asn] };
    const response = await fetch(`${API_URL}/charts/ip`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(filters)
...
async function loadCountryChartForAsn(canvasId, asn) {
    const filters = { ...currentFilters, asns: [asn] };
    const response = await fetch(`${API_URL}/charts/country`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function loadAsnChartForAsn(canvasId, asn) {
    const filters = { ...currentFilters, asns: [asn] };
    const response = await fetch(`${API_URL}/charts/asn`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function loadUsernameChartForAsn(canvasId, asn) {
    const filters = { ...currentFilters, asns: [asn] };
    const response = await fetch(`${API_URL}/charts/username`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
// Chart loading functions for Username comparison
async function loadDateChartForUsername(canvasId, username) {
    const filters = { ...currentFilters, usernames: [username] };
    const response = await fetch(`${API_URL}/charts/date`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(filters)
...
async function loadCountryChartForUsername(canvasId, username) {
    const filters = { ...currentFilters, usernames: [username] };
    const response = await fetch(`${API_URL}/charts/country`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function loadUnusualChartForUsername(canvasId, username) {
    const filters = { ...currentFilters, usernames: [username] };
    const response = await fetch(`${API_URL}/charts/unusual`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function loadIpChartForUsername(canvasId, username) {
    const filters = { ...currentFilters, usernames: [username] };
    const response = await fetch(`${API_URL}/charts/ip`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function loadAsnChartForUsername(canvasId, username) {
    const filters = { ...currentFilters, usernames: [username] };
    const response = await fetch(`${API_URL}/charts/asn`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function loadUsernameChartForUsername(canvasId, username) {
    const filters = { ...currentFilters, usernames: [username] };
    const response = await fetch(`${API_URL}/charts/username`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
...
async function loadCountryChartComparison(canvasId, timePeriod) {
    console.log(`[loadCountryChartComparison] Starting for ${canvasId}`, timePeriod);
    try {
        const filters = { ...currentFilters, date_from: timePeriod.from, date_to: timePeriod.to };
        console.log(`[loadCountryChartComparison] Fetching with filters:`, filters);
        ...
        const result = await response.json();
        console.log(`[loadCountryChartComparison] API response:`, result);
        
        if (charts[canvasId]) charts[canvasId].destroy();

        if (!result.success) {
            console.warn(`[loadCountryChartComparison] API returned success=false`);
            const ctx = document.getElementById(canvasId);
            charts[canvasId] = new Chart(ctx, {
                type: 'line',
        ...
        if (!result.data || result.data.length === 0) {
            console.warn(`[loadCountryChartComparison] No data returned (length: ${result.data ? result.data.length : 'null'})`);
            const ctx = document.getElementById(canvasId);
            charts[canvasId] = new Chart(ctx, {
                type: 'line',
                data: { labels: [], datasets: [] },
        ...
        console.log(`[loadCountryChartComparison] Processing ${result.data.length} data points`);
        const dateMap = {};
        const countries = new Set();
        result.data.forEach(row => {
            countries.add(row.country);
        ...
        const sortedCountries = Array.from(countries).sort((a, b) => countryTotals[b] - countryTotals[a]);
        console.log(`[loadCountryChartComparison] Creating chart with ${sortedCountries.length} countries`);
        ...
        const ctx = document.getElementById(canvasId);
        if (!ctx) {
            console.error(`[loadCountryChartComparison] Canvas element not found: ${canvasId}`);
            return;
        }
        ...
        console.log(`[loadCountryChartComparison] Chart created successfully for ${canvasId}`);
    } catch (error) {
        console.error(`[loadCountryChartComparison] Error loading country chart ${canvasId}:`, error);
        // Create error chart
        if (charts[canvasId]) charts[canvasId].destroy();
        const ctx = document.getElementById(canvasId);
        if (ctx) {
...
async function loadUnusualChartComparison(canvasId, timePeriod) {
    console.log(`[loadUnusualChartComparison] Starting for ${canvasId}`, timePeriod);
    try {
        const filters = { ...currentFilters, date_from: timePeriod.from, date_to: timePeriod.to };
        console.log(`[loadUnusualChartComparison] Fetching with filters:`, filters);
        ...
        const result = await response.json();
        console.log(`[loadUnusualChartComparison] API response:`, result);
        
        if (charts[canvasId]) charts[canvasId].destroy();

        if (!result.success) {
            console.warn(`[loadUnusualChartComparison] API returned success=false`);
            const ctx = document.getElementById(canvasId);
            charts[canvasId] = new Chart(ctx, {
                type: 'line',
                data: { labels: [], datasets: [] },
        ...
        if (!result.data || result.data.length === 0) {
            console.warn(`[loadUnusualChartComparison] No data returned (length: ${result.data ? result.data.length : 'null'})`);
            const ctx = document.getElementById(canvasId);
            charts[canvasId] = new Chart(ctx, {
                type: 'line',
                data: { labels: [], datasets: [] },
        ...
        console.log(`[loadUnusualChartComparison] Processing ${result.data.length} data points`);
        const dateMap = {};
        const countries = new Set();
        result.data.forEach(row => {
            countries.add(row.country);
        ...
        const sortedCountries = Array.from(countries).sort((a, b) => countryTotals[b] - countryTotals[a]);
        console.log(`[loadUnusualChartComparison] Creating chart with ${sortedCountries.length} countries`);
        ...
        const ctx = document.getElementById(canvasId);
        if (!ctx) {
            console.error(`[loadUnusualChartComparison] Canvas element not found: ${canvasId}`);
            return;
        }
        ...
        console.log(`[loadUnusualChartComparison] Chart created successfully for ${canvasId}`);
    } catch (error) {
        console.error(`[loadUnusualChartComparison] Error loading unusual chart ${canvasId}:`, error);
        // Create error chart
        if (charts[canvasId]) charts[canvasId].destroy();
        const ctx = document.getElementById(canvasId);
        if (ctx) {
